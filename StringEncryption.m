...
- (void)testSymmetricEncryption
{
	...
	NSString *_key = @"22515216714613359164620314164148123127244177";
	NSString *_secret = @"cpsych99@aol.co";
	NSData *_secretData = [_secret dataUsingEncoding:NSASCIIStringEncoding];
	
    NSLog(@"plaintext string v: %@",_secretData);
	NSLog(@"plaintext string: %@",[[NSString alloc] initWithData:_secretData encoding:NSASCIIStringEncoding]);
	
	CCOptions padding = kCCOptionPKCS7Padding;
	NSData *encryptedData = [self encrypt:_secretData key:[_key dataUsingEncoding:NSUTF8StringEncoding] padding:&padding];
	...
	NSData *data = [self decrypt:encryptedData key:[_key dataUsingEncoding:NSUTF8StringEncoding] padding:&padding];
			 ...
			 context:(CCOperation)encryptOrDecrypt padding:(CCOptions *)pkcs7
{
    ...
    //NSLog(@"pkcs7: %d", *pkcs7);
    //YC--NO NEED TO DO THIS. It will produc 24 character's encryption for 16 characters input
    //Always use kCCOptionPKCS7Padding
